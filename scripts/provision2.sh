    ...
    case $1 in
        ...
        --harbor-username)
            if [ -n "$2" ]; then
                ...
                printf 'ERROR: "--harbor-username" requires a non-empty value.\n' >&2
                usage
                exit 255
            fi
            ;;
        --harbor-password)
            if [ -n "$2" ]; then
                ...
                printf 'ERROR: "--harbor-password" requires a non-empty value.\n' >&2
                usage
                exit 255
            fi
            ;;
...
function deploy() {
  oc $ARG_OC_OPS new-project dev-$PRJ_SUFFIX   --display-name="Swagger Tasks - Dev"
  oc $ARG_OC_OPS new-project stage-$PRJ_SUFFIX --display-name="Swagger Tasks - Stage"
